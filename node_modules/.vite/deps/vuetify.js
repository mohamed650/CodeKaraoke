...
function useMask(props) {
  const mask = computed(() => {
    ...
    if (typeof props.mask === "string") {
      if (props.mask in presets)
        return presets[props.mask];
      return props.mask;
    }
  ...
  function maskValidates(mask2, char) {
    if (char == null || !isMask(mask2))
      return false;
    const item = tokens.value[mask2];
    if (item.pattern)
      return item.pattern.test(char);
  ...
  function maskText(text) {
    ...
    if (trimmedText == null)
      return "";
    if (!mask.value.length || !trimmedText.length)
      return trimmedText;
    let textIndex = 0;
    let maskIndex = 0;
    let newText = "";
  ...
  function unmaskText(text) {
    if (text == null)
      return null;
    if (!mask.value.length || !text.length)
      return text;
    let textIndex = 0;
    let maskIndex = 0;
    let newText = "";
    while (true) {
      ...
      if (maskValidates(mchar, tchar)) {
        ...
        while (true) {
          ...
          if (mchar2 == null || maskValidates(mchar2, tchar))
            break;
        }
        continue;
      }
